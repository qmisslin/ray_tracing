...
public:
	...
	plan3<T>(const vec3<T> &p, const vec3<T> &u, const vec3<T> &v)
	{
		vec3<T> n; n.cross(u,v);
		n.normalize(n.norm());
		a=n.X(); b=n.Y(); c=n.Z();
		d = -n.dot(p);
	}
	...
	plan3<T>(const vec3<T> &p, const vec3<T> &n)
	{
		a=n.X(); b=n.Y(); c=n.Z();
		d = -n.dot(p);
	}
