...
{
protected:
	vec3<T>		pt[2];	// intersection points
	T			dist[2];	// distance of point according to line vector
	int			n;		// number of intersections
public:
	// constructors
	intersection3<T>():n(0) { }
	intersection3<T>(const vec3<T> &x) { pt[0]=x; dist[0]=T(0); n=1; }
	intersection3<T>(const vec3<T> &x, const vec3<T> &y) { pt[0]=x; dist[0]=T(0); pt[1]=y; dist[1]=T(0); n=2; }
	intersection3<T>(const vec3<T> &x, T dd) { pt[0]=x; dist[0]=dd; n=1; }
	intersection3<T>(const vec3<T> &x, T dx, const vec3<T> &y, T dy) { pt[0]=x; dist[0]=dx; pt[1]=y; dist[1]=dy; n=2; }
	...
	vec3<T> valueOf() const { return pt[0]; }
	operator vec3<T>() { return pt[0]; }
	...
	{
		vec3<T> v; v.cross(li1.direction(),li2.direction());
		if (v.isNull()) { n=0; return false; }
		vec3<T> w=v; w.abs();
		vec3<T> dd; dd.PVec(li2.origin(), li1.origin());
		vec3<T> r; r.cross(li2.direction(), dd);
		T l;
		if ( w.X()>w.Y() && w.X()>w.Z() )
		{
			l = r.X()/v.X();
	...
	bool intersect(const vec3<T> &a, const vec3<T> &b, const plan3<T> &pl, T err)
	{
		vec3<T> dir; dir.PVec(a, b);
	...
	int intersectRay(const line3<T> &li, const vec3<T> &bbmin, const vec3<T> &bbmax)
	{
